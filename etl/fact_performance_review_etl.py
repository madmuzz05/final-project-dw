...
from config_etl import etl_config_fact_performance_review
from config_etl import dwh_conn
from config_etl import oltp_conn_pg_performance
...
def extract(table_config):
    ...
    try:
        ...
        with db_connection(oltp_conn_pg_performance) as conn:
...
def run_etl_fact_performance_review():
    """Run the full ETL process."""
    try:
        logging.info("Starting ETL Process...")
        validate_config(etl_config_fact_performance_review)  # Validate config
        for table_name, table_config in etl_config_fact_performance_review.items():
