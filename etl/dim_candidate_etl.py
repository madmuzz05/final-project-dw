...
from config_etl import etl_config_dim_candicate
...
def transform(df, table_config):
    ...
    try:
        ...
        df.drop(columns=["Position"], errors="ignore", inplace=True)
        df.drop(columns=["ApplicationDate"], errors="ignore", inplace=True)
        df.drop(columns=["Status"], errors="ignore", inplace=True)
        df.drop(columns=["InterviewDate"], errors="ignore", inplace=True)
        df.drop(columns=["OfferStatus"], errors="ignore", inplace=True)
...
def run_etl_dim_candidate():
    try:
        logging.info("Starting MongoDB ETL process...")
        validate_config(etl_config_dim_candicate)
        ...
        for table_name, table_config in etl_config_dim_candicate.items():
