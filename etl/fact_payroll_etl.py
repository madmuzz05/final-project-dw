...
from config_etl import etl_config_fact_payroll
from config_etl import dwh_conn
from config_etl import oltp_conn_pg_payroll
...
def extract(table_config):
    ...
    try:
        ...
        with db_connection(oltp_conn_pg_payroll) as conn:
...
def run_etl_fact_payroll():
    """Run the full ETL process."""
    try:
        logging.info("Starting ETL Process...")
        validate_config(etl_config_fact_payroll)  # Validate config
        for table_name, table_config in etl_config_fact_payroll.items():
